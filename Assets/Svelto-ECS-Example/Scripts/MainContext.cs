...
namespace Svelto.ECS.Example.Survive
{
	...
	public class Main : ICompositionRoot
	{
		...
		void SetupEnginesAndEntities()
		{
			...
			_enginesRoot.AddEngine(playerGrenadeEngine);
			//enemy engines
		...
		void BuildPlayerEntities(PrefabsDictionary prefabsDictionary)
		{
			...
			var grenadeSpawner = player.GetComponentInChildren<PlayerGrenadeSpanwerImplementor>();
			//var grenadeVFX = grenade.GetComponentInChildren<>();
			_entityFactory.BuildEntity<PlayerGunEntityDescriptor>(gun.gameObject.GetInstanceID(), new object[] { gun, grenadeSpawner });
			//_entityFactory.BuildEntity<PlayerGunEntityDescriptor>(grenade.gameObject.GetInstanceID(), new object[] {  })
        }
